...
int main(int argc, char ** argv)
{
	...
	*/
	...
	if(outflag){
		for(int i = 0; i < (int)inputfiles.size(); ++i){
			outputHuff(inputfiles[i], codes, outfilename);
		}
	...
	if(printSummary){
		...
		std::cout << "Word\t\t | Frequency\t | Huffman Code | Size" << std::endl;
		...
		for (map<string, int >::const_iterator it = frequenciesInput.begin(); it != frequenciesInput.end(); ++it)
		{
			...
			if(p1 != codes.end()){
				...
				std::cout << "\t|" << p1->second.size();
				if(p1->second.size() < shortestLength){
					shortestString = p1->first;
					shortestLength = p1->second.size(); 
				}
				std::cout << std::endl;
			}
		}
		std::cout << "shortest string: " << shortestString << ", size: " << shortestLength << std::endl;
	}
...
//void outputHuff(std::string filename, map<string, int> & frequencies, map<string, int> & frequenciesInput){
void outputHuff(std::string filename, map<string, vector<bool> > & codes, string outfilename){
	...
	while(!infile.eof())
	{
		...
		while( getline(stream, word, ' ') ){
			...
			if(word.length() > 0){
				map<string, vector<bool> >::const_iterator p1 = codes.find(word);
				if(p1 != codes.end()){
					std::copy(p1->second.begin(), p1->second.end(),
							std::ostream_iterator<bool>(outfile));
				}
